...
<div class="carousel-container" id="carousel" style="display: flex; flex-wrap: wrap; gap: 10px;">
  <!-- 图片将由 JavaScript 动态填充 -->
</div>

<script>
  const accessKey = "0Ukx3h0_C18RepNO7qXDQeFSfYQHh7Mr57cNmAN8X-M"; // 你在 Unsplash 注册的 Access Key
  ...
  async function fetchUnsplashImages() {
    try {
      for (let query of searchQueries) {
        const response = await fetch(`https://api.unsplash.com/photos/random?query=${query}&client_id=${accessKey}&orientation=landscape`);
        const data = await response.json();
        ...
        attribution.style.fontSize = "0.8rem";
        attribution.style.color = "#555";
        attribution.style.margin = "5px 5px 15px";
        attribution.innerHTML = `📷 Photo by <a href="${data.user.links.html}?utm_source=your_app_name&utm_medium=referral" target="_blank" rel="noopener">${data.user.name}</a> on <a href="https://unsplash.com?utm_source=your_app_name&utm_medium=referral" target="_blank" rel="noopener">Unsplash</a>`;
